...
class PermissionCategoryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('status', function($permissionCategory){
                return ($permissionCategory->status==1)?'<h5><span class="badge badge-primary">Active</span></h5>':
                '<h5><span class="badge badge-warning">Inactive</span></h5>';
            })
            ->addColumn('action', function ($permissionCategory) {
                $id = $permissionCategory->id;
                $btn = '';
                    $btn .= '<a href="'.route('permissioncategories.edit',$id).'"
                    class="btn btn-xs btn-info" data-toggle="tooltip" title="Edit">
                    <i class="fa fa-pen-alt"></i> </a> ';

                    // $btn .= '<a href="'.route('permissionCategorys.show',$id).'"
                    // class="btn btn-sm btn-secondary" data-toggle="tooltip" title="View">
                    // <i class="fa fa-eye"></i> </a> ';

                    if($permissionCategory->status==1)
                    {
                        $btn .='<a href="'.route('permissioncategories.inactive',$id).'"
                        class="btn btn-xs btn-danger" data-toggle="tooltip"
                        title="Suspend"><i class="fa fa-trash"></i> </a> ';

                    }elseif($permissionCategory->status==0)
                    {
                        $btn .='<a href="'.route('permissioncategories.activate',$id).'"
                        class="btn btn-xs btn-danger" data-toggle="tooltip"
